...
func Run(ctx context.Context, goal string, opts RunOptions) (finalAnswer string, err error) {
	...
	planner := &Planner{Model: opts.Model, SendReq: opts.SendReq}
	...
	for step := 0; step < opts.MaxSteps; step++ {
		...
		if opts.Debug {
			msg := fmt.Sprintf("decision: action=%s", decision.Action)
			if decision.Action == "tool" {
				msg += fmt.Sprintf(" name=%s args=%v", decision.Name, decision.Args)
			}
			if decision.Reasoning != "" {
				msg += fmt.Sprintf(" reasoning=%q", decision.Reasoning)
			}
			debugf("%s\n", msg)
		}
		...
		if tool == nil {
			...
			debugf("observation: unknown tool %s\n", decision.Name)
			continue
		}
