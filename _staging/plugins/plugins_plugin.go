...
func (p *PluginsPlugin) Register(k *kernel.Kernel) ([]*cobra.Command, error) {
	...
	listCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return tui.PrintBox(cmd.OutOrStdout(), "Plugins", strings.TrimRight(b.String(), "\n"))
		},
	}
	...
	enableCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, ok := k.Plugin(id); !ok {
				return tui.PrintError(cmd.ErrOrStderr(), fmt.Sprintf("Unknown plugin %q", id))
			}
			...
			viper.Set("plugins.enabled", enabled)
			viper.Set("plugins.disabled", disabled)
			if err := viper.WriteConfig(); err != nil {
				return err
			}
			return tui.PrintBox(cmd.OutOrStdout(), "Plugins", fmt.Sprintf("Enabled %s", id))
		},
	}
	...
	disableCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return tui.PrintBox(cmd.OutOrStdout(), "Plugins", fmt.Sprintf("Disabled %s", id))
		},
	}

	pluginsCmd.AddCommand(listCmd, enableCmd, disableCmd)
	return []*cobra.Command{pluginsCmd}, nil
}
